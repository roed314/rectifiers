...
\begin{proof}
Suppose $\varphi(\Fr) = y$, where $y$ is an element of $D$.  By our assumptions on $T$, we have that $\hat{H}^{-1}(\Gamma, T(L)) = \hat{H}^{-1}(\Gamma, X_*(T))$ (see \ref{cor:cohom_tori}).

Let $\lambda \in ker(\Nm : X_*(T) \rightarrow X_*(T))$.  We wish to show that $\xi(\lambda) = \xi'(\lambda)$.  Via the canonical identification $X_*(T) \cong X^*(\hat{T})$, we regard $\lambda$ as an element of $X^*(\hat{T})$.  Assume without loss of generality that we may write $y^n = \mu \otimes z$, for some $\mu \in X_*(\hat{T})$ and $z \in \mathbb{C}^{\times}$ (note that $y^n \in \hat{T}$).  By the local Langlands correspondence for tori, we obtain $\xi(\lambda) = z^{\langle \mu, \lambda \rangle}$, where $\langle , \rangle$ denotes the canonical pairing on $X_*(\hat{T}) \times X^*(\hat{T})$.

By definition of $\varphi'$, we have that $\varphi'(\Fr) = ty$, for some element $t \in \hat{T}$.  Let $w$ be the Weyl group element associated to $y$. Without loss of generality let us assume $t = \mu' \otimes z'$, where $\mu' \in X_*(\hat{T})$ and $z' \in \mathbb{C}^{\times}$.  Then
$$(ty)^n = t yty^{-1} y^2 t y^{-2} \cdots y^{n-1} t y^{1-n} y^n$$
$$= t w(t) w^2(t) \cdots w^{n-1}(t) y^n =(\Nm(\mu' ) \otimes z') \cdot (\mu \otimes z),$$ where here $\Nm$ denotes norm map on $\hat{T}$ corresponding to the given Galois action on $\hat{T}$.

We conclude again by the local Langlands correspondence for tori that $$\xi'(\lambda) = z^{\langle \mu, \lambda \rangle} (z' )^{\langle \Nm(\mu'), \lambda \rangle}.$$  Since $\langle , \rangle$ is Galois equivariant, $\langle \Nm(\mu'), \lambda \rangle = \langle\mu', \Nm(\lambda)\rangle$.
But $\lambda \in ker(\Nm : X_*(T) \rightarrow X_*(T))$, so $\langle\mu', \Nm(\lambda)\rangle = 0$.  Therefore, $\xi'(\lambda) = z^{\langle\mu, \lambda\rangle} = \xi(\lambda)$.
\end{proof}
