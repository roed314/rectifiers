...
Suppose now that $T \subset G$ for a connected reductive group $G$ over $K$.  We write $N$ for the normalizer of $T$ in $G$ and define $W = N/T$; set $\hat{N}$ as the normalizer of $\hat{T}$ in $\hat{G}$ and $\hat{W} = \hat{N}/\hat{T}$.  The identification of $X^*(T)$ and $X_*(\hat{T})$ yields a canonical anti-isomorphism between $W$ and $\hat{W}$.  Note that $W$ is a scheme over $K$; in general $W(K) \ne N(K) / T(K)$.
...
$K$-torus of $G$.
\begin{enumerate}
\item $N(G(L), T(K)) / T(L) \cong W(K)$.
\item The standard action of $N(G(L),T(L)) / T(L)$ on $T(L)$ determines
well-defined actions of $N(G(L), T(L))^{\Gamma} / T(K)$ and $W(K)$
...
\section{The relationship between the Gross construction and the DeBacker--Reeder and Reeder construction} \label{section:gross_debacker_reeder}
...
\S\ref{section:groups_of_type_L}.  DeBacker--Reeder and Reeder (in \cite{debackerreeder} and \cite{reeder}) associate a character $\chi_{\varphi}$ of $T(K)$ to $\varphi$.
...
\ref{section:groups_of_type_L}, $\varphi$ and $\varphi_{w}$ gives rise to characters
...
the local Langlands correspondence for tori (see section \ref{section:groups_of_type_L}).  Note that the above
...
We have therefore proven Theorem \ref{thm:unique_semisimple}.  In the case that $G$ is semisimple, there was nothing special about
...
more details, see \S\ref{section:groups_of_type_L}).
...
to $\Gamma$ (see~\S\ref{section:groups_of_type_L}).  Invariants and coinvariants
...
by any element of $W(K)$. (see Lemma \ref{lem:weyl_groups}). We say that two admissible pairs $(T, \xi)$ and $(T', \xi')$ are \emph{isomorphic} if there exists a $g \in G(K)$ such that $gT(K)g^{-1} = T'(K)$ and $\xi(t) = \xi'(gtg^{-1})$ for all $t \in T(K)$.  We denote by $P_G(K)$ the set of isomorphism classes of admissible pairs in $G$.
\end{definition}
...
\begin{definition}\label{def:regular_pair}
...
\begin{proposition}\label{prop:BH_result}
  Suppose that $(L/K, \xi)$ is an admissible pair such that $L/K$ is
  unramified of degree $n$ and $\xi|_{\OL} \equiv 1$.
  Then ${}_K \mu_{\xi}$ is unramified and
  ${}_K \mu_{\xi}(\varpi) = (-1)^{n-1}$.
...
>>>>>>> a98c424f630298fb7e9acbbc04eefa9d10565304
...
unramified.  By Proposition \ref{prop:BH_result}, we are done.
\end{proof}
...
as in Corollary \ref{cor:rectifier_agreement},
$\xi_{\varphi_{w}'}(\varpi) = a$.  By Proposition \ref{prop:BH_result}, we are
...
\section{Compatibility with DeBacker-Reeder} \label{section:DR_compat}
