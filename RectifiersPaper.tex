...
denotes the norm map from $L$ to $K$.  Then the image of $\varphi$
is contained in the normalizer of the dual split torus $\hat{T}$.  This normalizer
is an extension of $\Gal(L/K)$ by $\hat{T}$, which is a non-split extension.
...
where $L^1$ denotes the norm one elements in $L$.  We note that $L^1$
is an elliptic torus in $\PGL_{2}(K)$.  In particular, the Langlands parameter
$\varphi$ naturally provides a character $\chi$, not of the elliptic torus $L^1$,
but of the two-fold cover $\Lx / \Nm_{L/K}(\Lx)$.  To construct a supercuspidal
representation of $\PGL_{2}(K)$, we need a character of $L^1$.  Therefore,
to rectify the situation, we should twist $\chi$ by a genuine character of
$\Lx / \Nm_{L/K}(\Lx)$, in order to obtain a character of $L^1$.  This twist
is exactly what occurs in the work of \cite{bushnellhenniart}, and is the
subject of their notion of a rectifier.
...
We write $N$ for the normalizer of $T$ in $G$ and define $W = N/T$;
set $\hat{N}$ as the normalizer of $\hat{T}$ in $\hat{G}$ and
...
Let us recall the following notions from \cite[\S8]{bushnellhenniart}.
...
$\xi|_{1 + \mathfrak{p}_L^{i+1}}$ factors through the norm $N_{L/L_i}$,
and which is minimal for this property.  We say that $i \in \mathbb{Z}$
is a \emph{jump of} $\xi$ \emph{over} $F$ if $i \geq 1$ and $E_{i-1} \neq E_i$.

Suppose now that $(L/K,\xi)$ is an admissible pair, where $\xi$ has
level $r \geq 1$.  Suppose that $\xi|_{1 + \mathfrak{p}_L^r}$ does not
factor through $N_{L/L_i}$ for any proper subextension $L_i$ contained
in $L$, containing $K$.  Then $L_0 = L_1 = \cdots L_r = L$ and $L_{j} = K$
for all $j > r$.  By Theorem ??????????? and Theorem ??????
\MAxxx{This is the Ramification Theorem of Essentially Tame, III, but once
we find out what the error in the paper is, together with Remark 21 on
Essentially Tame, III, page 547},
we have
\MAxxx{Now state the proposition about rectifiers in this setting, based on the Ramification Theorem}

Now suppose that $(L/K, \xi)$ is an admissible pair, where $\xi$ has level
$r = 0$.  Then the set of jumps of $\xi$ over $F$ is empty.
Therefore, by \cite[Proposition 21]{bushnellhenniart}, we have
...
\end{proposition}
\MAxxx{We only showed that our rectifier agrees with B/H for depth zero!
We should do positive depth as well! (ie Reeder's setting, for $\GL_n$).
That is, this proposition only says what the B/H rectifiers are in
DeBacker/Reeder.  What are the B/H rectifiers in the Reeder setting?}
...
\S\ref{section:groups_of_type_L}.  DeBacker--Reeder and Reeder (in \cite{debackerreeder}
...
parameter.  Here, ${}^L G = \langle \hat{\theta} \rangle \ltimes \hat{G}$,
...
the local Langlands correspondence for tori (see section \ref{section:groups_of_type_L}).
...
We have therefore proven Theorem \ref{thm:unique_semisimple}.  In the
...
We recall that there is a notion of regularity for characters of $T(K)$
...
and $\varphi \mapsto L(\xi_{\varphi} \cdot {}_K \mu_{\xi_{\varphi}})$
...
of Reeder's positive depth paper.  We just need our definition of $P_G^{Reg}(K)$
...
there is a unique weak rectifier for $T$.  Moreover, ${}_K\mu_{\xi}$ is
unramified for all regular pairs $(T, \xi)$.
\end{theorem}
...
$\varphi_{w}'(\Fr)$ to be a representative of an elliptic element $\hat{w}$ in $\hat{W}$.
Then \cite[p. 824]{debackerreeder} and \cite[\S6]{reeder} imply that the characteristic
polynomial of $\varphi_{w}'(\Fr)$ is $X^n - a$, for some $a \in \CCx$.  One can see that,
...
so that $\varphi_{w}'(\Fr)$ is indeed the canonical lift of $\hat{w}$ to $\widetilde{W}$.

\section{Compatibility with DeBacker-Reeder} \label{section:DR_compat}

Let $\xi$ range over the set of characters of $T(L)_{\Gamma}$ such that $(T, \xi) \in P_G^{Reg}(K)$, and let $\xi \mapsto {}_K \mu_{\xi}^i$ be a rectifier for $T \subset G$, $i = 1,2$.  By hypothesis, we have $$L({}_K \mu_{\xi}^1 \xi) = L({}_K \mu_{\xi}^1 \xi).$$ 
By \cite[\S10]{murnaghan}, there exists $w_{\xi} \in W(K)$, depending on $\xi$, such that $$({}_K \mu_{\xi}^1 \xi)^{w_{\xi}} = {}_K \mu_{\xi}^2 \xi.$$  Suppose that $\xi$ is not trivial on $T(1 + \mathfrak{p}_L)_{\Gamma}$.  Restricting the equation $({}_K \mu_{\xi}^1 \xi)^{w_{\xi}} = {}_K \mu_{\xi}^2 \xi$ to $T(1 + \mathfrak{p}_L)_{\Gamma}$, we get that $\xi^{w_{\xi}} = \xi$, by condition (1) of Definition \ref{def:rectifier}.  This contradicts admissibility of $\xi$.

Now suppose that $\xi$ is trivial on $T(1 + \mathfrak{p}_L)_{\Gamma}$.  Consider the restriction of the equation $({}_K \mu_{\xi}^1 \xi)^{w_{\xi}} = {}_K \mu_{\xi}^2 \xi$ to $T(\mathcal{O}_L)_{\Gamma} \cong T(\mathcal{O}_K)$.  Write $\mu = (({}_K \mu_{\xi}^2)^{w_{\xi}})^{-1} {}_K \mu_{\xi}^2$. Reducing mod $\mathfrak{p}_L$, we conclude that there exists a character, $\mu$, of $T(k)$ with the property that if $\xi$ is any $W(k)$-regular character of $T(k)$, then $\mu \xi$ is $W(k)$-conjugate to $\xi$. By ???????, $\mu$ is trivial.  Therefore, $({}_K \mu_{\xi}^1)^{w_{\xi}} = {}_K \mu_{\xi}^2$ upon restriction to $T(\mathcal{O}_L)_{\Gamma}$.  Finally, since ${}_K \mu_{\xi}^1 \xi$ and ${}_K \mu_{\xi}^2 \xi$ must descend to $T(K)$ by condition (2) of Definition \ref{def:rectifier}, we have that ${}_K \mu_{\xi}^1$ and ${}_K \mu_{\xi}^2$ have the same restriction to $\hat{H}^{-1}(\Gamma, T(L))$.  Hence, ${}_K \mu_{\xi}^1 = {}_K \mu_{\xi}^2$
