...
We initiate a program to generalize the rectifier of Bushnell and Henniart,
which occurs in the local Langlands correspondence for
$\GL_{n}(K)$, to regular elliptic Langlands parameters for
unramified connected reductive groups. %WRITE
...
necessarily contained within the $L$-group of a maximal torus, they need to make certain adjustments in order to produce a character of an elliptic torus.  The different authors remedy this situation in various ways.

For example, consider the group $G = \PGL_{2}(K)$, and suppose that
$\varphi : \Weil_K \rightarrow \SL_{2}(\CC)$ is an
irreducible representation.  Then $\varphi$ is a Langlands parameter corresponding to a supercuspidal representation of $\PGL_2(K)$.  Moreover, the image of $\varphi$
...
$\varphi = \Ind_{\Weil_L}^{\Weil_K}(\chi)$ \cite[Section 34]{bushnell-henniart:06a}.  In particular, $\chi$ is a character of the group $\Lx / \Nm_{L/K}(\Lx)$.
...
$\Lx / \Nm_{L/K}(\Lx)$ that is nontrivial on $\Kx$.  One can then attach a supercuspidal representation of $\PGL_2(K)$ to $\chi'$ via the construction of Bushnell and Kutzko \cite{bushnell-kutzko:AdmissibleDual}. The tensoring character that gives the correct supercuspidal representation of $\PGL_2(K)$
is precisely what appears in Bushnell and Henniart's rectifier.  In \cite{bushnell-henniart:10a}, Bushnell and Henniart compute this rectifier in the \emph{essentially tame} setting of $\GL_n(K)$.
...
$\varphi \mapsto \pi_{\xi \cdot \mu_{\xi}}$ is the local Langlands correspondence for $\GL_n(K)$.  In the specific case of supercuspidal representations of $\GL_2(K)$ with trivial central character (i.e. supercuspidal representations of $\PGL_2(K)$), one computes that the character $\mu_{\xi}$ is, as discussed above, a character of $\Lx / \Nm_{L/K}(\Lx)$ that is nontrivial on $\Kx$.
...
We would like to remark that the notion of a covering group of a torus occuring in the local Langlands correspondence is an old one, dating back at least to work of Adams and Vogan \cite{adams-vogan:91} in the setting of real groups.  In the theory of real groups, an admissible homomorphism $W_{\mathbb{R}} \rightarrow {}^L G$ automatically has image inside the normalizer of a torus in ${}^L G$.  As such, it naturally produces a genuine character $\chi$ of the \emph{$\rho$-cover of $T(\mathbb{R})$}, denoted $T(\mathbb{R})_{\rho}$, a certain double cover of $T(\mathbb{R})$.  To $\chi$, one can naturally attach a collection of representations of $G(\mathbb{R})$ to construct an $L$-packet.
...
\begin{proposition}[{\cite[Thm. 1.1]{bertapelle-gonzalez:13b}}]
...
\begin{example}
\begin{enumerate}
\item Each minisotropic torus in $\GL_n$ is of the form $\Res_{L/K} \Gm$ for a degree $n$ extension $L$ of $K$.

\item Let $T$ be a torus in $\GSp_4$ \cite{morris:91a}*{Prop. 1.3}, split over a quartic unramified extension $L/K$ with $\Gal(L/K) = \langle \tau \rangle$, with cocharacter lattice
\begin{align*}
...
We now review the theory of groups of type L \cite{}*{\S 7}.
...
$\ker(\Nm : T(L) \rightarrow T(K))$.  We do so by expressing the quotient
$\varphi' \cdot \varphi^{-1}$ as a product
\begin{equation}
\label{eq:prod_phii}
\textstyle{\prod_{i=0}^{n-1} \varphi_i,}
\end{equation}
where the $\varphi_i$ satisfy 
\begin{equation}
\label{eq:Fr_phii}
\varphi_i(\gamma) = \Fr^i(\varphi_0(\gamma))
\end{equation}
for all $\gamma \in \Gal(\bar{L}/L)$. The quotient $\xi' \cdot \xi^{-1}$ will then break up
analogously, allowing the computation of its value on $x \in T(L)$ given $\Nm(x)$.

We define $\varphi_i$ separately on inertia and on $\Fr^n$.
Set $t = \varphi'(\Fr)\varphi(\Fr)^{-1} \in \hat{T}$ and set $\varphi_i(\Fr^n) = \Fr^i(t)$.
The restriction of $\varphi' \cdot \varphi^{-1}$ to inertia is a map $\chi : \I \to \hat{T}$.
Let $\chi_0$ be an arbitrary $(1+q+\cdots+q^{n-1})$-th root of $\chi$
\textcolor{red}{(such a $\chi_0$ can only exist if the exponent of the image of $\chi$ is relatively prime to 
$(1+q+\cdots+q^{n-1})$-th root.  The tame part of the inertia can divide $q-1$)}, and define
$\varphi_i$ on $\I$ to be $\Fr^i \circ \chi_0$.  The $\varphi_i$ satisfy \eqref{eq:Fr_phii}
by definition.  They satisfy \eqref{eq:prod_phii} by the definition of $\chi_0$, and on
$\Fr^n$ by the following computation.  If $g = \varphi(\Fr) \in D$, then
\begin{align*}
\varphi'(\Fr^n) \varphi(\Fr^n)^{-1} &= (tg)^n g^{-n} \\
&= \textstyle{\prod_{i=0}^{n-1} g^i t g^{-i}} \\
&= \textstyle{\prod_{i=0}^{n-1} \Fr^i(t)}
\end{align*}
since $g$ projects to $\Fr \in \Gamma$.

Now let $\xi_i$ be the character of $T(L)$ associated to $\varphi_i$.
By \cite[Lem. 4.3.1]{reeder-debacker:09a}, $\xi_i = \xi_0 \circ \Fr^i.$
Suppose that $x \in T(L)$ with $\Nm(x) = 1$.  Then
\begin{align*}
\xi'(x) \xi(x)^{-1} &= \textstyle{\prod_{i=0}^{n-1} \xi_i(x)} \\
&= \textstyle{\xi_0 \left(\prod_{i=0}^{n-1}\Fr^i(x)\right)} \\
&= 1.\\
\end{align*}
\end{proof}
...
is a pinning as in \cite[\S 3.1]{reeder:10a}.
...
$H \subset G$, a natural relationship between the rectifiers for tori in $H$ and $G$ would allow us to apply the results of \cite{bushnell-henniart:10a} to rectifiers for general groups.
\end{enumerate}
\end{remark}
