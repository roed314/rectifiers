...
$p$-adic field $F$.  Let $\varphi : W_F \rightarrow {}^L G$ be a
Langlands parameter for $G(F)$, and suppose that $\varphi$ factors
...
Restricting $\varphi$ to $W_E$, by the local Langlands correspondence for
...
\item $\xi \cdot {}_F \mu_{\xi} \in \widehat{T(F)}$, the pair $(T, \xi \cdot {}_F \mu_{\xi})$
 is regular \MAxxx{Need to define notion of regularity for characters
    of tori}, and $\varphi \mapsto L(\xi_{\varphi} \cdot {}_F \mu_{\xi_{\varphi}})$
...
$\varphi \mapsto L(\xi_{\varphi} \cdot {}_F \mu_{\xi_{\varphi}})$
...
a Langlands parameter $\varphi_o :W _F \rightarrow {}^L G$ by
...
$\varphi_o$ to be trivial on $I_F$.
If $G$ is semisimple,
we will prove that the function $$(T, \xi) \mapsto \xi_{\varphi_o}$$
satisfies conditions (1)-(4), and we may therefore set
${}_F \boldsymbol\mu^{\mathrm{Reg}}(T,\xi) = \xi_{\varphi_o}$.
Moreover, we prove compatibility with Bushnell/Henniart:

\begin{theorem}
  If $G = GL(n,F)$, the function $(T,\xi) \mapsto \xi_{\varphi_o}$ agrees with
  the rectifier of Bushnell/Henniart, when $(T,\xi) \in Reg(F)$.  Moreover, $\varphi_o$ is the
  unique \MAxxx{up to a sign, or something like that?} Langlands
  parameter such that $\xi_{\varphi_o} = {}_F \mu_{\xi}$.
\end{theorem}
...
\begin{align*}
\Nm_{E/F} : S(E) &\rightarrow S(F) \\
s &\mapsto \prod_{\sigma \in \Gal(E/F)} \sigma(s).
\end{align*}
...
Restricting $\varphi$ to $W_E$ we get a homomorphism
$$\varphi|_{W_E} : W_E \rightarrow \hat{T}.$$
By the Langlands correspondence for tori, this gives us a character
$\chi : T(E) \rightarrow \mathbb{C}^\times$.  Since $\varphi|_{W_E}$ extends
...
Langlands parameter $\varphi$. We note that $T(E)_{\Gamma}$ is a cover of
...
$\varphi_2(\Fr) = t \varphi_1(\Phi_F)$ for some $t \in \hat{T}$.
...
Let $\chi_{\varphi}$ be the character of $T(F) = T^{\Phi_{\sigma}}$ that
...
where $\Gamma = \Gal(E/F)$.  Recall that $\varphi$ canonically gives
...
obtained from $\varphi|_{W_E}$ via the local Langlands correspondence, one can see
from the constructions in \cite{debackerreeder} and \cite{reeder} that
$\chi_{\varphi} \circ \Nm = \chi$ on $T(\mathcal{O}_E)_{\Gamma}$ (one can see
...
Let $\varphi$ be a RSELP.  Then $\varphi(I_F) \subset \hat{T}$ and
...
\begin{definition}
Given $\varphi$, we define a homomorphism $\varphi_o : W_F \rightarrow {}^L G$ by
\begin{enumerate}
\item $\varphi_o|_{I_F} \equiv 1$
...
$\xi_{\varphi}$ and $\xi_{\varphi_o}$ of $T(E)_{\Gamma}$, respectively.
...
Since $\varphi_o|_{I_F} \equiv 1$, $\xi_{\varphi_o}$ is trivial on
$T(\mathcal{O}_E)_{\Gamma}$.  By lemma
\ref{grossanddebackerreedercompatibility}, we have that
$(\xi_{\varphi} \otimes \xi_{\varphi_o}^{-1})|_{T(\mathcal{O}_E)_{\Gamma}} = \chi_{\varphi}|_{T(\mathcal{O}_F)}$.
...
$\xi_{\varphi} \otimes \xi_{\varphi_o}^{-1}$ is a character of $T(F)$.  Since
$\xi_{\varphi} \otimes \xi_{\varphi_o}^{-1}$ is correct on $T(\mathcal{O}_E)_{\Gamma}$, and
...
for any Langlands parameter $\varphi'$ such that $\varphi'|_{I_F} \equiv 1$
...
$\xi_{\varphi_o}(\varpi_E) = (-1)^{n-1}$, because of Proposition \ref{powersoflifts}.
\qed
...
$\varphi_o' : W_F \rightarrow GL(2,\mathbb{C})$ by $\varphi_o'|_{I_F} \equiv 1$ and

\[
\varphi_o'(\Phi_F) = \left( \begin{array}{cc}
0 & a\\
b & 0
\end{array} \right)
\]
...
$\xi_{\varphi_o'}(\varpi_E) = b$.  By Lemma \ref{rectifierofbushnellhenniart}, we are 
forced to have $b = -1$.
