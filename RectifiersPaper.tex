...
$\varphi_{\hat{w}}'(\Fr)$ to be a representative of an elliptic element $\hat{w}$ in $\hat{W}$.  Then \cite[p. 824]{debackerreeder} and \cite[???]{reeder} \MAxxx{Which page?  It's page 31 of Reeder's version of the paper on his webpage} imply that the characteristic polynomial of $\varphi_{\hat{w}}'(\Fr)$ is $X^n - a$, for some $a \in \mathbb{C}^{\times}$.  One can see that, by analogous arguments
