...
Bushnell and Henniart define a rectifier as follows.
Suppose that $\varphi$ is an \emph{essentially tame} supercuspidal Langlands parameter for
$\GL_n(K)$ (see \S\ref{section:BH_recall}).  The local Langlands correspondence for tori yields a canonical character $\xi$
of $L^{\times}$, where $L/K$ is some degree $n$ separable extension.  There is a character
...
We generalize their notion of rectifier to other reductive groups $G$ by replacing $L^\times$ with various
groups of points associated to tori in $G$.
Suppose that $\varphi : \Weil_K \rightarrow {}^L G$ is a Langlands parameter
that factors through the normalizer of a maximal torus (see \S\ref{section:general_rectifiers}).
The local Langlands for tori again provides a canonical way to proceed.  Assuming a mild
cohomological condition (see \S\ref{section:general_rectifiers}), one
obtains from $\varphi$ a character of a cover of an elliptic torus.
The rectifier $\mu_{\xi}$ is then a character of this cover such that
...
to \emph{admissible} characters of this cover (see \S\ref{section:general_rectifiers}).
