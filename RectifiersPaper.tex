...
if there is some $\alpha \in Q_T$ with $\mu_\xi' = \alpha \mu_\xi$ for all admissible $(T, \xi)$.

\end{definition}

Since we have assumed $\HT{0}(\Gamma,T(L)) = 0$, the condition that $\xi \cdot \mu_\xi$
descends to $T(K)$ is equivalent to it vanishing on $\HT{-1}(\Gamma, T(L))$.  Moreover, if
...
\end{align*}
which attaches,
  to each $(T, \xi) \in \Pgen(K)$, a character
  $\mu_{\xi}$ of $T(L)_{\Gamma}$, satisfying conditions (1)-(3)
  of Definition \ref{def:rectifier}.
\end{enumerate}
We say that two weak rectifiers $\mugen$ and $(\mugen)'$ for $T$ are equivalent if there is an $\alpha \in Q_T$ with $(\mugen)' = \alpha\mugen$.
\end{definition}
