...
\begin{proof}
Suppose $\varphi(\Fr) = y$, where $y$ is an element of $D$.  By our assumptions
on $T$, we have that $\HT{-1}(\Gamma, T(L)) = \HT{-1}(\Gamma, X_*(T))$ (see \ref{cor:cohom_tori}).

Let $\lambda \in \ker(\Nm : X_*(T) \rightarrow X_*(T))$.  We wish to show that
$\xi(\lambda) = \xi'(\lambda)$.  Via the canonical identification $X_*(T) \cong X^*(\hat{T})$,
we regard $\lambda$ as an element of $X^*(\hat{T})$.  Assume without loss of generality
that we may write $y^n = \mu \otimes z$, for some $\mu \in X_*(\hat{T})$ and
$z \in \mathbb{C}^{\times}$ (note that $y^n \in \hat{T}$).  By the local Langlands
correspondence for tori, we obtain $\xi(\lambda) = z^{\langle \mu, \lambda \rangle}$,
where $\langle , \rangle$ denotes the canonical pairing on $X_*(\hat{T}) \times X^*(\hat{T})$.

By definition of $\varphi'$, we have that $\varphi'(\Fr) = ty$, for some element
$t \in \hat{T}$.  Let $w$ be the Weyl group element associated to $y$. Without
loss of generality let us assume $t = \mu' \otimes z'$, where $\mu' \in X_*(\hat{T})$
