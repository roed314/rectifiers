...
ourselves to cases where general supercuspidal $L$-packets have been constructed.  
...
We will show that the characters ${}_F \mu_{\xi}$ that arise are actually unramified 
(i.e. trivial on $T(\mathfrak{o}_E)_{\Gamma}$).  In particular, the setting 
...
Let $Reg(T, \xi)$ denote the set of admissible pairs that arise from RSELPs,
as in \cite{debackerreeder} and \cite{reeder}.  We can prove:

\begin{theorem}
Let $G$ be semisimple, and $T$ as in Definition \ref{rectifierdefinition}.  
If ${}_F \boldsymbol\mu$ is a rectifier for $T$, then
${}_F \boldsymbol\mu|_{Reg(T,\xi)}$ is uniquely determined.
\end{theorem}

Once phrased in this manner, uniqueness is a straightforward matter
(see \S\ref{}) \MAxxx{Fix when we can.}. 

Moreover, we construct a function on $Reg(T,\xi)$ that satisfies conditions
(1)-(4) above.  As explained earlier, this is all that we can hope to do, 
...
\begin{theorem}
  ...
  \xi_o$$ satisfies conditions (1)-(4) of Definition \ref{rectifierdefinition}.
\end{theorem}
