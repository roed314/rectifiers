...
In this paper, we sidestep half of the problem by assuming that $\HT{0}(\Gamma, T(L)) = 0$.  This condition holds for unramified minisotropic tori in $\GL_n$
...
For many $G$ other than $\GL_n$, % what are the conditions on G and the characters?
there are constructions of supercuspidal $L$-packets
$\Lpack(\chi)$ associated to characters $\chi$ of elliptic tori $T(K)$.  However, a Langlands parameter $\varphi$ does not naturally
...
The constructions $\chi \mapsto \Lpack(\chi)$ of supercuspidal $L$-packets we use are those of DeBacker/Reeder and Reeder. We define rectifiers for
