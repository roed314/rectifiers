...
pairs $(T, \xi)$.  For example, if $G = GL(n)$, ${}_F \mu_{\xi}$ can be ramified,
even if $T$ is unramified.  We return to this case when supercuspidal $L$-packets
are more generally constructed to include this case.
...
the rectifier in the situation where supercuspidal $L$-packets are understood.

\begin{definition}\label{regularpairs}
Let $Reg(T, \xi)$ denote the set of admissible pairs $(T,\xi)$ such that 

(i) $T$ is $F$-minisotropic.  
...
construct the rectifier in general.  
...
This Langlands parameter $\phi_o$ does not depend on $G$ being
semisimple.  We believe that Theorem \ref{regularrectifierdefinition}
can be proven for arbitrary $G$, using the obvious generalized definition of
$\xi_o$, but are not able to prove this.  However, we prove:

\begin{theorem}
  If $G = GL(n,F)$, the function $(T,\xi) \mapsto \xi_o$ agrees with
  the rectifier of Bushnell/Henniart, when $(T,\xi) \in Reg(T,\xi)$.  Moreover, $\phi_o$ is the
