...
Let $\chi$ and $\chi'$ be the associated characters of $T(L)_{\Gamma}$.

\begin{lemma}\label{toralmodification}
$\chi$ and $\chi'$ have the same restriction to $\HT{-1}(\Gamma, T(L))$.
\end{lemma}

\begin{proof}
It suffices to prove that $\chi' \cdot \chi^{-1}$ vanishes on
...
by $\varphi_i(\sigma) = 1$ for $\sigma \in I_L$ and
$\varphi_i(\Fr^n) = \Fr^i(t)$; let $\chi_i$ be the associated character
of $T(L)$.  By \cite[Lem. 4.3.1]{debackerreeder}, $\chi_i = \chi_0 \circ \Fr^i.$
Suppose that $x \in T(L)$ with $\Nm(x) = 1$.  Then
\begin{align*}
\chi'(x) \chi(x)^{-1} &= \prod_{i=0}^{n-1} \chi_i(x) \\
&= \chi_0 \left(\prod_{i=0}^{n-1}\Fr^i(x)\right) \\
&= 1.\\
\end{align*}
\end{proof}
