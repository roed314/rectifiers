...
Let $G$ be a connected reductive group, defined over a $p$-adic field $K$.  The local Langlands conjecture predicts the existence of a finite to one map, from the set of isomorphism classes of irreducible admissible representations of $G(K)$, to the set of equivalence classes of admissible homomorphisms from the Weil-Deligne group $W_K'$ of $K$, to the $L$-group ${}^L G$ of $G$.

In the case of supercuspidal representations of $G(K)$, there has been a significant amount of progress in the local Langlands correspondence in recent years.  In the recent work of \cite{bushnellhenniart}, \cite{debackerreeder}, \cite{kaletha}, and \cite{reeder}, the strategy of constructing a local Langlands correspondence is to first attach a character of an elliptic torus to a Langlands parameter $W_K \rightarrow {}^L G$, where $W_K$ is the Weil group of $K$, and then to construct a putative supercuspidal $L$-packet associated to this character.

Some of the difficulty arises in the construction of the character of an elliptic torus.  The difficulty arises from the fact that a Langlands parameter $W_K \rightarrow {}^L G$ does not necessarily have image inside the $L$-group of an elliptic torus.  If it did, then the local Langlands correspondence for tori would give us the desired character.  Instead, one has to rectify the situation in some way.

The simplest example of this difficulty arising is in the case of supercuspidal representations of $PGL(2,K)$.  Suppose that $p \neq 2$, and that that $\varphi : W_K \rightarrow SL(2,\mathbb{C})$ is a homomorphism that is irreducible as a representation.  Then $\varphi = Ind_{W_L}^{W_K}(\chi)$, where $L/K$ is a tamely ramified quadratic extension, for some character $\chi$ of $L^{\times}$ that is trivial on $N_{L/K}(L^{\times})$, where $N_{L/K}$ denotes the norm map from $L$ to $K$.  Then the image of $\varphi$ is contained in the normalizer of the dual split torus $\hat{T}$.  This normalizer is an extension of $Gal(L/K)$ by $\hat{T}$, which is a non-split extension.  In particular, the image of $\varphi$ is not contained in the $L$-group of a torus.

Moreover, we have the exact sequence $$1 \rightarrow \mathbb{Z} / 2 \mathbb{Z} \rightarrow L^{\times} / N_{L/K}(L^{\times}) \rightarrow L^1 \rightarrow 1,$$ where $L^1$ denotes the norm one elements in $L$.  We note that $L^1$ is an elliptic torus in $PGL(2,K)$.  In particular, the Langlands parameter $\varphi$ naturally provides a character $\chi$, not of the elliptic torus $L^1$, but of the two-fold cover $L^{\times} / N_{L/K}(L^{\times})$.  To construct a supercuspidal representation of $PGL(2,K)$, we need a character of $L^1$.  Therefore, to rectify the situation, we should twist $\chi$ by a genuine character of $L^{\times} / N_{L/K}(L^{\times})$, in order to obtain a character of $L^1$.  This twist is exactly what occurs in the work of \cite{bushnellhenniart}, and is the subject of their notion of a rectifier.
...
Let $(L/K,\xi)$ be an admissible pair, and let $i \in \mathbb{Z}, i \geq 0$.  Then there is a unique sub-extension $L_i/K$ of $L/K$ such that $\xi|_{1 + \mathfrak{p}_L^{i+1}}$
...
a Langlands parameter $\varphi_{w} :W _K \rightarrow {}^L G$ by
...
\begin{theorem}
  If $G = GL(n,K)$, the function $(T,\xi) \mapsto \xi_{\varphi_{w}}$ agrees with
...
\section{Compatibility with Bushnell-Henniart}\label{GL(n)}
...
Suppose we define
$\varphi_{w}' : W_K \rightarrow GL(n,\mathbb{C})$ by $\varphi_{w}'|_{I_K} \equiv 1$ and
$\varphi_{w}'(\Fr)$ to be a representative of an elliptic element $\hat{w}$ in $\hat{W}$.  Then \cite[p. 824]{debackerreeder} and \cite[\S6]{reeder} imply that the characteristic polynomial of $\varphi_{w}'(\Fr)$ is $X^n - a$, for some $a \in \mathbb{C}^{\times}$.  One can see that, by analogous arguments
