...
\MAxxx{Add to rectifier paper: For GL(n) : The condition
$\phi(\chi) \mapsto \pi(\chi \mu_{\chi})$ doesn't determine $\mu_{\chi}$
uniquely, because of $GL_2(F_3)$. So B/H make a definition of rectifier
in order to prove uniqueness. However : generalizing this definition to
unramified groups, you do not get uniqueness, because of $SL_2(F_3)$!
So something else is needed. We do not explore this in this paper.
(Wait If you don't get uniqueness for $SL_2(F_3)$, do you also not
get uniqueness for $PGL_2(F_3)$ for similar reasons? That would be
weird, because you should get uniqueness for $PGL_2(F_3)$, since you
do for $GL_2(F_3)$.....right? SOmething's weird. Maybe we made a
mistake for $SL_2(F_3)$?? Or maybe you don't get uniqueness for
$PGL_2(F_3)$? Or maybe you do get uniqueness for $PGL_2(F_3)$?? )}
...
is exactly what appears in the work of Bushnell and Henniart, and is the
...
a new notion of admissible pair.   We will show that a rectifier exists, and for $q$
...
This means, in particular, that in the depth zero case, the rectifier is canonical.
...
We remark that our rectifier agrees with that of \cite{moy} in the positive depth setting.

We now present an outline of the paper.  In \S\ref{section:BH_recall}, we recall
the notion of rectifier as in \cite{bushnellhenniart}, and we describe
...
the rectifier of \cite{bushnellhenniart} in the setting of depth zero
...
sum of characters.  Let us recall the construction of the
...
Let $T$ be a torus defined over $K$.    Let $K_n$ \MAxxx{Bad notation.  We already
defined $K_n$ in the notation section.  Fix this for all bad $K_n$'s.}  be the maximal
...
ramification.  Let $\TT_r$ \MAxxx{Why the notation with an $r$?  Doesn't $r$ usually
have something to do with filtration?} be the integral model of $T$ defined in Yu's
minimal congruent filtration \MAxxx{there is another integral model of $T$ other than
the Neron model?}, and let $\{T(K)_r\}_{r \ge 0}$ and
$\{T(K_n)_r\}_{r \ge 0}$ be the corresponding filtrations of $T(K)$ and
$T(K_n)$.

Let $\C$ be the scheme of
connected components of $\TT$ \MAxxx{should this be $\TT$ or $\TT_r$?}, 
...
in section \ref{section:general_rectifiers}.
...
\MAxxx{Is this really true?  I don't see it immediately.}  It is more
convenient however, for our purposes, to begin by fixing a torus
$T$, rather than a Langlands parameter.
...
We note that the definition of admissible pair above generalizes
...
\begin{conjecture} \label{conj:unique_rectifier}
  ...
  \mu_{\xi}$.
\end{conjecture}\MAxxx{We probably shouldn't state a conjecture which
is false....right?}
...
\begin{definition}
Let $T$ be as in Definition \ref{def:rectifier}.  We say that $T$ satisfies 
condition $\star \star$ if the following holds: given any character $\alpha$ 
of $T(k)$, there exists a regular character $\beta$ of $T(k)$ whose order 
is not divisible by the order of $\alpha$.
\end{definition}

\begin{theorem} \label{thm:unique_semisimple}
Let $G$ be semisimple, and $T$ as in Definition \ref{def:rectifier}, satisfying
condition $\star \star$.  Then
...
satisfies condition (2).  Moreover, since $\varphi_{\hat{w}}|_{I_K} \equiv 1$,
$\xi_{\varphi_{\hat{w}}}^{-1}$ is unramified, so this function also
...
$$(\mu_\xi \cdot \xi)^{w_\xi} = \mu'_\xi \cdot \xi.$$
Suppose that $\xi$ is not trivial on $T(\PL)_{\Gamma}$.
Restricting the equation
$(\mu_\xi \cdot \xi)^{w_\xi} = \mu'_\xi \cdot \xi$
to $T(\PL)_{\Gamma}$, we get that $\xi^{w_{\xi}} = \xi$,
...
Write $\lambda = ((\mu_\xi)^{w_\xi})^{-1} \cdot \mu'_\xi$.
...
$W(k)$-conjugate to $\xi$. Suppose the order of $\lambda$ is $n > 1$.
Since $T$ satisfies $\star \star$, there exists
a regular character $\chi$ of $T(k)$, whose order, say $m$, is not divisible by $n$.

We have $\lambda \chi = \chi^{w_{\chi}}$.  $\chi^{w_{\chi}}$
also has order $m$.  Raising the equation $\lambda \chi = \chi^{w_{\chi}}$
to the power $m$, we get $\lambda^{n-m} \equiv 1$.  Since $n$ does not divide $m$,
we get $\lambda^r \equiv 1$, for some $0 < r < n$, contradicting the
definition of $n$.  Therefore, $\lambda \equiv 1$.

Therefore, $(\mu_\xi)^{w_{\xi}}|_{T(\OL)_{\Gamma}} = \mu'_\xi|_{T(\OL)_{\Gamma}}$.
But we have already constructed
a rectifier.  Therefore, we may reduce ourselves to showing that any other
...
$(\mu_\xi)^{w_{\xi}}|_{T(\OL)_{\Gamma}} = \mu'_\xi|_{T(\OL)_{\Gamma}} 1$
...
that $T$ be unramified, since not all supercuspidal representations can be derived
from unramified tori.  Even in the Debacker-Reeder and Reeder contexts, tori in non-semisimple
