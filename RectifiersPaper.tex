...
\MAxxx{Should this be $(T, \xi) \mapsto \xi_{\varphi_o}^{-1}$????  Since the inverse is what we twist by to get a character of $T(K)$ (see Section \ref{mainresults})}
...
We have thus constructed a character $\xi_{\varphi}$ of $T(L)_{\Gamma}$ from a
...
\S\ref{groupsoftypeL}.  DeBacker and Reeder associate a character $\chi_{\varphi}$
of $T(K)$ to $\varphi$.
...
$\xi_{\varphi}|_{T(\OL)_{\Gamma}}$ as a character of
...
Let $\varphi$ be a regular semisimple elliptic Langlands parameter.
Then $\varphi(I_K) \subset \hat{T}$ and
$\varphi(\Fr) = \hat{\theta} f$ for some $f \in \hat{N}$, where $\hat{\theta}$
is the dual Frobenius automorphism (see \cite[\S 3]{debackerreeder}).  Let $\hat{w}$
be the image of $f$ in $\hat{W}$.
Write $\hat{w} = s_{\alpha_1} \cdots s_{\alpha_n}$
...
\begin{definition}
Given $\varphi$, we define a homomorphism $\varphi_o : W_K \rightarrow {}^L G = \langle \hat{\theta} \rangle \ltimes \hat{G}$ by
\begin{enumerate}
...
\begin{proposition}
If $G$ is semisimple, then $\xi_{\varphi} \otimes \xi_{\varphi_o}^{-1} = \chi_{\varphi} \circ \Nm$.
\end{proposition}
...
$(\xi_{\varphi} \otimes \xi_{\varphi_o}^{-1})|_{T(\OL)_{\Gamma}} = \chi_{\varphi} \circ \Nm|_{T(\OL)_{\Gamma}} $.  \MAxxx{Maybe I should now say something like :
``Since $\HT{-1}(\Gamma, T(L)) \neq 0$, there exists a unique extension
of $\xi_{\varphi_o}^{-1}|_{T(\OL)_{\Gamma}}$ to a character $\lambda$ of $T(L)_{\Gamma}$
...
However, we will see that the Tits group lift is forced upon us when we
...
where $\sigma$ is the element of $\Gamma$ that cyclically shifts one space to the right.  Thus $\ker(\Nm) \subset I_{\Gamma}(X_*(T))$.
\end{proof}
...
that $T(K) = \{(w, \sigma(w), \sigma^2(w), \cdots, \sigma^{n-1}(w)) : w \in L^{\times} \}$, where
...
Finally, since $\varphi_o|_{I_F} \equiv 1$, we get that $\xi_{\varphi_o}(\varpi_L)$ is
unramified.  By Proposition \ref{rectifierofbushnellhenniart}, we are done.
\end{proof}

We end this section by explaining why the Tits group lift of $\tilde{w}$ is forced upon us.
...
$\xi_{\varphi_o'}(\varpi_E) = b$.  By Proposition \ref{rectifierofbushnellhenniart}, we are
forced to set $b = -1$, so that $\varphi_o'(\Fr)$ is the lift of $\hat{w}$ to $\widetilde{W}$.\MAxxx{I think that we should do the general $GL(n)$ case here.  I'm not convinced yet that the Tits group lift is even forced upon us for general $GL(n)$...it looks like it might be, though.}
