...
happen to be unramified (i.e. trivial on $T(\mathcal{O}_L)_{\Gamma}$).  This implies,
...
$\varphi_2(\Fr) = t \varphi_1(\Phi_K)$ for some $t \in \hat{T}$.
...
$\varphi_1(\Phi_L) = \varphi_1(\Phi_K)^r$, where $r$ is the
order of $\Gamma$.
...
Let $\chi_{\varphi}$ be the character of $T(K) = T^{\Phi_{\sigma}}$ that
...
$$1 \rightarrow \HT{-1}(\Gamma, T(\mathcal{O}_L)) \rightarrow T(\mathcal{O}_L)_{\Gamma}
  \rightarrow T(\mathcal{O}_K) \rightarrow \HT{0}(\Gamma, T(\mathcal{O}_L)) \rightarrow 1$$

Moreover, by Lemmas \ref{H1compactpart} and \ref{H0compactpart}, we have
$\HT{-1}(\Gamma, T(\mathcal{O}_L)) = \HT{0}(\Gamma, T(\mathcal{O}_L)) = 1$.
Therefore, the map
$$T(\mathcal{O}_L)_{\Gamma} \xrightarrow{\Nm} T(\mathcal{O}_K)$$
is an isomorphism, and we may view
$\chi|_{T(\mathcal{O}_L)_{\Gamma}}$ as a character of
$T(\mathcal{O}_K)$ via this isomorphism.  Now, since $\chi$ was
...
$\chi_{\varphi} \circ \Nm = \chi$ on $T(\mathcal{O}_L)_{\Gamma}$ (one can see
...
\begin{lemma}\label{grossanddebackerreedercompatibility}
  The restriction to $T(\mathcal{O}_L)_{\Gamma} \xrightarrow{\sim} T(\mathcal{O}_K)$
  of the genuine character arising from the Gross construction
  coincides with the character of $T(\mathcal{O}_K)$ that is
...
$\varphi(\Phi) = \hat{\theta} f$ for some $f \in \hat{N}$.  Let $\hat{w}$
...
\item $\varphi_o(\Phi) = \hat{\theta} \tilde{w}$
\end{enumerate}
\end{definition}
...
Note that $T(K) = T(\mathcal{O}_K)$, and so in particular
$T(\mathcal{O}_L)_{\Gamma}$ surjects onto $T(K)$ via the norm map
$\Nm$.  This, together with the fact that $\HT{-1}(\Gamma, T(L)) \neq 0$,
gives us that $\HT{-1}(\Gamma,T(L))$ and
$T(\mathcal{O}_L)_{\Gamma}$ together generate $T(L)_{\Gamma}$.

Since $\varphi_o|_{I_K} \equiv 1$, $\xi_{\varphi_o}$ is trivial on
$T(\mathcal{O}_L)_{\Gamma}$.  By lemma
\ref{grossanddebackerreedercompatibility}, we have that
$(\xi_{\varphi} \otimes \xi_{\varphi_o}^{-1})|_{T(\mathcal{O}_L)_{\Gamma}} = \chi_{\varphi}|_{T(\mathcal{O}_K)}$.
Therefore, $\xi_{\varphi} \otimes \xi_{\varphi_o}^{-1}$ is correct on
$T(\mathcal{O}_L)_{\Gamma}$.  Moreover, by Lemma
...
$\xi_{\varphi} \otimes \xi_{\varphi_o}^{-1}$ is correct on $T(\mathcal{O}_L)_{\Gamma}$, and
since $T(\mathcal{O}_L)_{\Gamma} \xrightarrow{\Nm} T(K)$ is surjective,
we are done.
\qed
...
and $\varphi'(\Phi) = w'$, where $w'$ was any lift of $\hat{w}$ to $N$.
...
\begin{lemma}\label{rectifierofbushnellhenniart}
  ...
  unramified of degree $n$ and $\xi|_{\mathcal{O}_L} \equiv 1$.
...
\proof
As $T = Res_{L/K}(GL_1)$, $\Gamma$ acts on $X_*(T)$ by cyclic shift.
...
Since $\Phi_L = \Phi_K^n$ and $\Phi_L$ corresponds to $\varpi_L$ under local
...
\[
\varphi_o'(\Phi_K) = \left( \begin{array}{cc}
0 & a\\
b & 0
\end{array} \right)
\]
