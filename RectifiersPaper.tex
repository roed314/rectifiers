...
$$N_{K/k} : X \otimes \left( \mathcal{O}_K^\times / (1 + \mathfrak{p}_K) \right) \rightarrow (X \otimes \left( \mathcal{O}_K^\times / (1 + \mathfrak{p}_K) \right) )^G$$
which is just
...
$\HH^1(G, X\otimes K^{\times}) = \HH^1(G, X) = ker(N_{K/k}) / (1 - g)X$
...
$$\mathrm{Hom}(\mathbb{T}^{\Phi_{\sigma}},
  ...
  \mathrm{Hom}_{\Phi_{\sigma}, 1}(\mathbb{T}^{\Phi_{\sigma}^n},
  \mathbb{C}^\times) \stackrel{\sim}{\rightarrow}
  \mathrm{Hom}_{\Phi_{\sigma}, 1}(X \otimes \mathfrak{f}_n^\times, \mathbb{C}^\times)$$
...
Therefore, the map
$$T(\mathcal{O}_E)_{\Gamma} \xrightarrow{N} T(\mathcal{O}_F)$$
...
get that $\chi_{\phi} \circ N = \chi$ on $T(\mathcal{O}_E)_{\Gamma}$.
...
since $T(\mathcal{O}_E)_{\Gamma} \xrightarrow{N} T(F)$ is surjective,
