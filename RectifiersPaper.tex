...
appears in the rectifier of Bushnell and Henniart.
...
a new notion of admissible pair.   We will show that a rectifier exists and, assuming
that $q$ is sufficiently large, is
unique. We will then show that our rectifier is compatible
...
Therefore, in the positive depth setting, a rectifier depends on the
...
from ``admissible'' characters of this cover.  This is the idea behind our rectifier, an extension
of Bushnell and Henniart's.
...
DeBacker-Reeder and Reeder.  In \S\ref{section:general_rectifiers}, we
...
we expound some natural questions that arise in our work.

\subsection*{Acknowledgements}

This paper has benefited from conversations with Gordan Savin, Jeffrey Adams,
Geo Kam Fai, Andrew Fiori, and Guy Henniart.  We thank them all.
...
\section{From characters to L-packets}
...
characters: two regular and two irregular.  Since the two regular characters are interchanged
...
For general $G$ we will call a character of $T(K)$ \emph{regular} if it is not fixed by any nontrivial element
of the rational Weyl group $W(K)$ (see \cite[p. 802]{debackerreeder} and \cite[\S3]{reeder}).
Note that this notion is sometimes referred to as strongly regular, with the term regular
reserved for characters fixed by a reflection.  The depth zero character of the split torus in $\SL_3(\QQ_7)$ inflated from
$$\begin{pmatrix} 3^x & & \\ & 3^y & \\ & & 3^{-x-y} \end{pmatrix} \mapsto \zeta_6^{2x + 4y}$$
is fixed by an 3-cycle.
...
\begin{definition} \label{def:rectifier}
  ...
  which attaches to each admissible pair $(T, \xi) \in P_G(K)$, a character $
...
that $\xi \cdot \mu_{\xi}$ descends to $T(K)$ is possible, so
condition (2) makes sense.  We believe that Definition \ref{def:rectifier} may
be modified to be applicable in the case that $\HT{0}(\Gamma,T(L)) \neq 0$
(see \S\ref{section:questions}). \DRxxx{Fix phrasing}

\begin{conjecture} \label{conj:unique_rectifier}\MAxxx{Now we can remove the condition ``If $q$ is sufficiently large''.  Do this here.}
  Let $T$ be as in Definition \ref{def:rectifier}.  If $q$ is sufficiently large, then $T$
  admits a unique rectifier $\bmu : (T, \xi) \mapsto
  \mu_{\xi}$.
\end{conjecture}

We note that the elliptic torus in $SL_2(\mathbb{Q}_3)$ presents an immediate obstruction to the
conjecture's validity
for arbitrary $q$, and the reason for this revolves around a curious property of $SL_2(\mathbb{F}_3)$.
The question of placing additional conditions on the definition of rectifier
that will guarantee uniqueness for all $q$ is the subject of
future work.

We also note that as the local Langlands correspondence is not known in general, we must restrict
...
We will show that the characters $\mu_{\xi}$ that arise in this setting
...
For example, if $G = \GL_n$, $\mu_{\xi}$ can be ramified,
even if $T$ is unramified.  Since supercuspidal $L$-packets have not been constructed in the generality that
...
is not fixed by any element of $W(K)$.\MAxxx{In other words, this definition generalizes
the definition of MINIMAL admissible pair of Bushnell/Henniart it seems.  HMMMMMM....this seems 
interesting and weird.  We should say that an admissible pair satisfying 
this condition here will be called a \emph{minimal admissible pair}.  Then, after the
definition, we should say that this is an analogue of Bushnell and Henniart's
minimal admissible pair.}  
We denote by $\Pgen(K)$ the set
of admissible pairs in $G$ in general position.
\item A \emph{weak rectifier} for $T \subset G$ is a function
\begin{align*}
\mugen : (T, \xi) \mapsto \mu_{\xi}
\end{align*}
which attaches,
  to each admissible pair $(T, \xi) \in \Pgen(K)$, a character
...
Let $G$ be semisimple, and $T$ as in Definition \ref{def:rectifier}.  Then there exists
a weak rectifier for $T$.  If $q$ is sufficiently large, then
this weak rectifier is the unique weak rectifier for $T$. \DRxxx{fix phrasing}
\end{theorem}
...
Suppose that $\xi$ is not trivial on $T(\PL)_{\Gamma}$.
Restricting the equation
${}^{w_\xi} (\mu_\xi \cdot \xi) = \mu'_\xi \cdot \xi$
to $T(\PL)_{\Gamma}$, we get that ${}^{w_{\xi}} (\xi) = \xi$,
by condition (1) of Definition \ref{def:rectifier}.
Since $\xi$ is admissible, we get that $w_{\xi} = 1$, which implies
that $\mu_{\xi} = \mu'_{\xi}$.

Now suppose that $\xi$ is trivial on $T(\PL)_{\Gamma}$.
Consider the restriction of the equation
${}^{w_\xi} (\mu_\xi \cdot \xi) = \mu'_\xi \cdot \xi$
to $T(\OL)_{\Gamma} \cong T(\OK)$.
Write $\lambda = ({}^{w_\xi} (\mu_\xi))^{-1} \cdot \mu'_\xi$.
Reducing mod $\PL$, we conclude that there exists a character,
$\lambda$, of $T(k)$, with the property that if $\xi$ is any
$W(k)$-regular character of $T(k)$, then $\lambda \xi$ is
$W(k)$-conjugate to $\xi$. Therefore, by Lemma \ref{}, \MAxxx{FIll this in}
...
We also note for emphasis that even though $ \mugen((T, \xi))$ is
unramified for pairs $(T, \xi) \in \Pgen(K)$, it will not be
unramified for all admissible pairs $(T, \xi) \in P_G(K)$, as noted earlier.

Finally, we note that nowhere in the proof of Theorem \ref{thm:unique_semisimple}
did we use condition (3) of Definition \ref{def:rectifier}.  We have
included this condition in Definition \ref{def:rectifier} since it appears in
Definition \ref{rectifierbushnellhenniart}.  We expect that this condition will
be used when we consider more general tori and more general admissible pairs.

\section{Compatibility with Bushnell-Henniart} \label{section:BH_compat}

In this section, we show that our function $\mugen$
...
\begin{theorem}
  ...
  the rectifier of Bushnell/Henniart, when $(T,\xi) \in \Pgen(K)$ such that $\xi$ is trivial
  on $T(\PL)_{\Gamma}$.
\end{theorem}
