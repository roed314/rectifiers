...
\MAxxx{I guess we don't show compatibility with their rectifier.  So fix this claim everywhere
in the paper.  But explain that it's ok, because the rectifier depends on various
constructions, and can be different, probably depending on the construction of
supercuspidals}
...
the rectifier in the settings that we will need.  In \S\ref{section:padic_tori},
...
state and prove the main theorems of this paper.
In \S\ref{section:BH_compat}, we show that our rectifier is compatible with
the rectifier of \cite{bushnellhenniart}.  Finally, in \S\ref{section:questions},
...
$\mathfrak{p}_K$ the maximal ideal in $\OK$,
...
in this paper we will write $\mu_\xi$ for ${}_K \mu_{\xi}$.  The function
...
in $L$, containing $K$.  Then $\mu_{\xi}$ is unramified and
  $\mu_{\xi}(\varpi) = (-1)^{n-1}$.
\end{proposition}
...
what to fill it in with}, $\lambda$ is trivial.
Therefore, $(\mu_\xi)^{w_{\xi}} = \mu'_\xi$
upon restriction to $T(\OL)_{\Gamma}$.
