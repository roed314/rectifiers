...
$\GL_{n}(K)$.  Our generalization is to arbitrary connected reductive
...
Let $G$ be a connected reductive group, defined over a $p$-adic field $K$.  The local Langlands conjecture predicts the existence of a finite to one map, from the set of isomorphism classes of irreducible admissible representations of $G(K)$, to the set of equivalence classes of admissible homomorphisms from the Weil-Deligne group $\WD_K$ of $K$, to the $L$-group ${}^L G$ of $G$.

In the case of supercuspidal representations of $G(K)$, there has been a significant amount of progress in the local Langlands correspondence in recent years.  In the recent work of \cite{bushnellhenniart}, \cite{debackerreeder}, \cite{kaletha}, and \cite{reeder}, the strategy of constructing a local Langlands correspondence is to first attach a character of an elliptic torus to a Langlands parameter $\Weil_K \rightarrow {}^L G$, where $\Weil_K$ is the Weil group of $K$, and then to construct a putative supercuspidal $L$-packet associated to this character.

Some of the difficulty arises in the construction of the character of an elliptic torus.  The difficulty arises from the fact that a Langlands parameter $\Weil_K \rightarrow {}^L G$ does not necessarily have image inside the $L$-group of an elliptic torus.  If it did, then the local Langlands correspondence for tori would give us the desired character.  Instead, one has to rectify the situation in some way.

The simplest example of this difficulty arising is in the case of supercuspidal representations of $\PGL_{2}(K)$.  Suppose that $p \neq 2$, and that that $\varphi : \Weil_K \rightarrow \SL_{2}(\CC)$ is a homomorphism that is irreducible as a representation.  Then $\varphi = \Ind_{\Weil_L}^{\Weil_K}(\chi)$, where $L/K$ is a tamely ramified quadratic extension, for some character $\chi$ of $L^{\times}$ that is trivial on $\Nm_{L/K}(L^{\times})$, where $\Nm_{L/K}$ denotes the norm map from $L$ to $K$.  Then the image of $\varphi$ is contained in the normalizer of the dual split torus $\hat{T}$.  This normalizer is an extension of $\Gal(L/K)$ by $\hat{T}$, which is a non-split extension.  In particular, the image of $\varphi$ is not contained in the $L$-group of a torus.

Moreover, we have the exact sequence $$1 \rightarrow \mathbb{Z} / 2 \mathbb{Z} \rightarrow L^{\times} / \Nm_{L/K}(L^{\times}) \rightarrow L^1 \rightarrow 1,$$ where $L^1$ denotes the norm one elements in $L$.  We note that $L^1$ is an elliptic torus in $\PGL_{2}(K)$.  In particular, the Langlands parameter $\varphi$ naturally provides a character $\chi$, not of the elliptic torus $L^1$, but of the two-fold cover $L^{\times} / \Nm_{L/K}(L^{\times})$.  To construct a supercuspidal representation of $\PGL_{2}(K)$, we need a character of $L^1$.  Therefore, to rectify the situation, we should twist $\chi$ by a genuine character of $L^{\times} / \Nm_{L/K}(L^{\times})$, in order to obtain a character of $L^1$.  This twist is exactly what occurs in the work of \cite{bushnellhenniart}, and is the subject of their notion of a rectifier.

In this paper, we propose a generalization of their notion of a
...
with the rectifiers of $\GL_{n}(K)$ as in \cite{bushnellhenniart}.
...
If $\chi : K^{\times} \rightarrow \mathbb{C}^{\times}$ is a character, we define the \emph{level} of $\chi$ to be the smallest integer $r$ such that $\chi|_{1 + \mathfrak{p}_K^{r+1}} \equiv 1$, with $\chi|_{1 + \mathfrak{p}_K^{r+1}} \not\equiv 1$.
...
\section{Rectifiers for $\GL_{n}(K)$}

We recall the notion of a rectifier, as in \cite{bushnellhenniart}.
...
essentially tame case, an irreducible representation $\varphi : \Weil_K
\rightarrow \GL_{n}(\CC)$ naturally provides an \emph{admissible
  pair} $(L/K, \xi)$.  Here, $L/K$ is a degree $n$ separable extension
and $\xi$ is a character (with certain conditions) of $L^{\times}$,
which one can view as a maximal elliptic torus in $\GL_{n}(K)$.  Howe
...
We will need a description of the characters ${}_K \mu_{\xi}$ in certain cases.  Let us recall the following notions from \cite[\S8]{bushnellhenniart}.

Let $(L/K,\xi)$ be an admissible pair, and let $i \in \mathbb{Z}, i \geq 0$.  Then there is a unique sub-extension $L_i/K$ of $L/K$ such that $\xi|_{1 + \mathfrak{p}_L^{i+1}}$ factors through the norm $N_{L/L_i}$, and which is minimal for this property.  We say that $i \in \mathbb{Z}$ is a \emph{jump of} $\xi$ \emph{over} $F$ if $i \geq 1$ and $E_{i-1} \neq E_i$.

Suppose now that $(L/K,\xi)$ is an admissible pair, where $\xi$ has level $r \geq 1$.  Suppose that $\xi|_{1 + \mathfrak{p}_L^r}$ does not factor through $N_{L/L_i}$ for any proper subextension $L_i$ contained in $L$, containing $K$.  Then $L_0 = L_1 = \cdots L_r = L$ and $L_{j} = K$ for all $j > r$.  By Theorem ??????????? and Theorem ??????\MAxxx{This is the Ramification Theorem of Essentially Tame, III, but once we find out what the error in the paper is, together with Remark 21 on Essentially Tame, III, page 547}, we have \MAxxx{Now state the proposition about rectifiers in this setting, based on the Ramification Theorem}

Now suppose that $(L/K, \xi)$ is an admissible pair, where $\xi$ has level $r = 0$.  Then the set of jumps of $\xi$ over $F$ is empty.  Therefore, by \cite[Proposition 21]{bushnellhenniart}, we have
...
\end{proposition}
\MAxxx{We only showed that our rectifier agrees with B/H for depth zero! We should do positive depth as well! (ie Reeder's setting, for $\GL_n$).  That is, this proposition only says what the B/H rectifiers are in DeBacker/Reeder.  What are the B/H rectifiers in the Reeder setting?}
...
Let $\varphi : \Weil_K \rightarrow {}^L G$ be a regular semisimple elliptic Langlands parameter.  Here, ${}^L G = \langle \hat{\theta} \rangle \ltimes \hat{G}$, where $\hat{\theta}$ is the dual Frobenius automorphism on $\hat{G}$ (see \cite[\S 3]{debackerreeder}).
...
\begin{definition}
Given $\hat{u} \in \hat{W}$, let $\tilde{u}$ be its canonical lift to $\widetilde{W}$.  We define a homomorphism $\varphi_{\hat{u}} : \Weil_K \rightarrow {}^L G$ by
\begin{enumerate}
...
\ref{groupsoftypeL}, $\varphi$ and $\varphi_{w}$ gives rise to characters
...
$\xi_{\varphi} \otimes \xi_{\varphi_{w}}^{-1} = \chi_{\varphi} \circ \Nm$ on each of these two subgroups.
...
\section{Rectifiers for general reductive groups}

One may ask whether rectifiers exist for groups apart from $\GL_{n}(K)$.
Even moreso, how does one define the notion of rectifier for
groups apart from $\GL_{n}(K)$?  The answer can be gleamed from a recent
...
of $\Weil_K$.  Suppose that $T$ splits
over the Galois extension $L$ of $K$, and set $\Gamma = \Gal(L/K)$.
Since $\varphi|_{\Weil_L}$ has image in $\hat{T} = X^*(T) \otimes \CCx$,
...
Let $T$ be a $K$-minisotropic torus, that splits over an unramified
  extension $L$.  Suppose $\xi$ is a character of $T(L)_{\Gamma}$.
The pair $(T, \xi)$ is called \emph{admissible} if $\xi$ is not fixed
by any element of $(N(G,T)/T)(K)$. (see Lemma \ref{weylgroups}). We say that two admissible pairs $(T, \xi)$ and $(T', \xi')$ are \emph{isomorphic} if there exists a $g \in G(K)$ such that $gT(K)g^{-1} = T'(K)$ and $\xi(t) = \xi'(gtg^{-1})$ for all $t \in T(K)$.  We denote by $P_G(K)$ the set of isomorphism classes of admissible pairs in $G$.
\end{definition}

We recall that there is a notion of regularity for characters of $T(K)$ (see \cite[p. 802]{debackerreeder} and \cite[\S3]{reeder}).
...
\item The character $\xi \cdot {}_K \mu_{\xi}$ descends to $T(K)$, is regular, and $\varphi \mapsto L(\xi_{\varphi} \cdot {}_K \mu_{\xi_{\varphi}})$
...
Suppose $(T, \xi)$ is an admissible pair.
\begin{enumerate}
...
\item An admissible pair is \emph{regular} if $\xi|_{T(1+\mathfrak{p}_L^{r})_{\Gamma}}$
is not fixed by any element of $(N(G,T)/T)(K)$.  We denote by $P_G^{Reg}(K)$ the set
of regular pairs in $G$.
\item A \emph{weak rectifier} is a function
\begin{align*}
{}_K\boldsymbol\mu^{\mathrm{Reg}} : (T, \xi) \mapsto {}_K \mu_{\xi}
\end{align*}
on $P_G^{Reg}(K)$, satisfying conditions (1)-(3) of Definition \ref{rectifierdefinition}.
\end{enumerate}
\end{definition}
\MAxxx{IMPORTANT: These 3 conditions certainly fall into the set of admissible pairs considered in DeBacker/Reeder and Reeder.  We don't need to find conditions on admissible pairs $(T,\xi)$ that are equivallent to the 3 conditions on Langlands parameters that are considered on page 31 of Reeder's positive depth paper.  We just need our definition of $P_G^{Reg}(K)$ to be a subset of their pairs $(T,\xi)$ so that condition (3) of Theorem \ref{regularrectifierdefinition} makes sense. Note that David has an argument that condition (iii) is equivalent (or implied by?) condition (2) on page 31 of Reeder's positive depth paper.}

We can prove:

\begin{theorem}\label{regularrectifierdefinition}
Let $G$ be semisimple, and $T$ as in Definition \ref{rectifierdefinition}.  Then there is a unique weak rectifier for $T$.  Moreover, ${}_K\mu_{\xi}$ is unramified for all regular pairs $(T, \xi)$.
\end{theorem}
...
sending Frobenius to a canonical lift of $\varphi_{w}(\Fr)$, using the Tits group, and by setting
$\varphi_{w}$ to be trivial on $I_K$.
If $G$ is semisimple,
we will prove that the function $$(T, \xi) \mapsto \xi_{\varphi_{w}}^{-1}$$
satisfies conditions (1)-(3), and we may therefore set
${}_K \boldsymbol\mu^{\mathrm{Reg}}(T,\xi) = \xi_{\varphi_{w}}$.
Moreover, we prove compatibility with Bushnell/Henniart:

\begin{theorem}
  If $G = \GL_{n}(K)$, the function $(T,\xi) \mapsto \xi_{\varphi_{w}}$ agrees with
  the rectifier of Bushnell/Henniart, when $(T,\xi) \in P_G^{Reg}(K)$.  Moreover, $\varphi_{w}$ is the
  unique Langlands
  parameter such that $\xi_{\varphi_{w}} = {}_K \mu_{\xi}$.
\end{theorem}
...
\section{Compatibility with Bushnell-Henniart}\label{GLn}

In this section, we show that our function ${}_K
  \boldsymbol\mu^{\mathrm{Reg}}$ agrees with the rectifier of Bushnell/Henniart.
...
Let $\hat{w}$ be a Coxeter element of $\GL_{n}(\CC)$.  Let $\tilde{w}$ be the canonical lift of $\hat{w}$ to $\widetilde{W}$. Then $\tilde{w}^n = (-Id)^{n-1}$, where $Id$
...
By \cite[\S 2.4]{serre1}, $\varpi$ corresponds to $\Fr^n$ under the Artin reciprocity map for $L$, so we obtain

\begin{corollary}\label{agreemenetofrectifiers}
$\xi_{\varphi_{w}} = {}_K \mu_{\xi}$.
\end{corollary}
...
$\sigma$ is a generator of $\Gal(L/K)$.  A uniformizer $\varpi$ in $L^{\times}$
...
$\xi_{\varphi_{w}}(\varpi) = (-1)^{n-1}$, because of Proposition \ref{powersoflifts}.
...
Suppose we define
$\varphi_{w}' : \Weil_K \rightarrow \GL_{n}(\CC)$ by $\varphi_{w}'|_{I_K} \equiv 1$ and
$\varphi_{w}'(\Fr)$ to be a representative of an elliptic element $\hat{w}$ in $\hat{W}$.  Then \cite[p. 824]{debackerreeder} and \cite[\S6]{reeder} imply that the characteristic polynomial of $\varphi_{w}'(\Fr)$ is $X^n - a$, for some $a \in \CCx$.  One can see that, by analogous arguments
as in Corollary \ref{agreemenetofrectifiers},
$\xi_{\varphi_{w}'}(\varpi) = a$.  By Proposition \ref{rectifierofbushnellhenniart}, we are
forced to set $a = (-1)^{n-1}$.  Finally, one can show by an inductive argument that the canonical lift $\tilde{w}$ to $\widetilde{W}$ has characteristic polynomial $X^n - (-1)^{n-1}$, so that $\varphi_{w}'(\Fr)$ is indeed the canonical lift of $\hat{w}$ to $\widetilde{W}$.

\section{Compatibility with DeBacker-Reeder}
