...
by any element of $(N(G,T)/T)(K)$. (see Lemma \ref{weylgroups}). We say that two admissible pairs $(T, \xi)$ and $(T', \xi')$ are \emph{isomorphic} if there exists a $K$-isomorphism $\alpha$ from $T$ to $T'$ with $\xi = \xi' \circ \alpha$.  We denote by $P_G(K)$ the set of isomorphism classes of admissible pairs in $G$.
\end{definition}
...
we need in order to prove this conjecture, we prove Theorem \ref{regularrectifierdefinition}, which determines
the rectifier in the situation where supercuspidal $L$-packets have been
constructed.

\begin{definition}\label{regularpairs}
Let $P_G^{Reg}(K)$ denote the set of admissible pairs $(T,\xi)$ such that

(i) $\xi$ is trivial on $T(1+\mathfrak{p}_L^{r+1})_{\Gamma}$ but nontrivial on
$T(1+\mathfrak{p}_L^{r})_{\Gamma}$ for some $r \geq 0$.

(ii) $\xi|_{T(1+\mathfrak{p}_L^{r})_{\Gamma}}$ is not fixed by any element of
...
There exists a unique function $${}_K
  \boldsymbol\mu^{\mathrm{Reg}} : P_G^{Reg}(K) \mapsto {}_K \mu_{\xi}$$ which attaches,
  to each admissible pair $(T, \xi) \in P_G^{Reg}(K)$, a character ${}_K
  \mu_{\xi}$ of $T(L)_{\Gamma}$ satisfying the following conditions.
...
\item The character $\xi \cdot {}_K \mu_{\xi}$ descends to $T(K)$, is regular, and $\varphi \mapsto L(\xi_{\varphi} \cdot {}_K \mu_{\xi_{\varphi}})$ agrees with \cite{debackerreeder} and \cite{reeder}

\item If $(T, \xi_i) \in P_G^{Reg}(K)$ are admissible pairs  such that
...
lift canonical?} lift of $\varphi_{w}(\Fr)$, using the Tits group, and by setting
$\varphi_{w}$ to be trivial on $I_K$.
If $G$ is semisimple,
we will prove that the function $$(T, \xi) \mapsto \xi_{\varphi_{w}}$$
\MAxxx{Should this be $(T, \xi) \mapsto \xi_{\varphi_{w}}^{-1}$????  Since the inverse is what we twist by to get a character of $T(K)$ (see Section \ref{mainresults})}
...
$\varphi_{w}'(\Fr)$ to be a representative of an elliptic element $\hat{w}$ in $\hat{W}$.  Then \cite[p. 824]{debackerreeder} and \cite[???]{reeder} \MAxxx{Which page?  It's page 31 of Reeder's version of the paper on his webpage} imply that the characteristic polynomial of $\varphi_{w}'(\Fr)$ is $X^n - a$, for some $a \in \mathbb{C}^{\times}$.  One can see that, by analogous arguments
