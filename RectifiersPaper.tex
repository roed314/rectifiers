...
\end{align*}
\MAxxx{Sometimes we write $\Nm_{E/F}$, sometimes we
...
Moy and Prasad \cite{moyprasad1} \MAxxx{Is this the correct reference?}
...
$\Gamma = \Gal(L/K)$.\MAxxx{Umm.....does $E/F$ need to be Galois?  I asked
...
\DRxxx{Yes, $L/K$ is necessarily Galois.  I'll tell you why on Skype sometime.}
\begin{enumerate}
...
$\hat{W} = \hat{N} / \hat{T}$.  Here, $\alpha$ range over the set of simple
roots of $\hat{T}$ in $\hat{B}$.
...
$\sigma_{\alpha} = \phi_{\alpha}\mat{0}{1}{-1}{0}$.\MAxxx{Is this last paragraph completely correct?  Maybe we should do Lie algebra version.}
...
$(\xi_{\varphi} \otimes \xi_{\varphi_{\hat{w}}}^{-1})|_{T(\OL)_{\Gamma}} = \chi_{\varphi} \circ \Nm|_{T(\OL)_{\Gamma}} $.  \MAxxx{Maybe I should now say something like :
``Since $\HT{-1}(\Gamma, T(L)) \neq 0$, there exists a unique extension
of $\xi_{\varphi_{\hat{w}}}^{-1}|_{T(\OL)_{\Gamma}}$ to a character $\lambda$ of $T(L)_{\Gamma}$
such that $\xi_{\varphi} \otimes \lambda$ factors to a character of $T(K)$.''  This
seems to make things more clear to the reader, and may be the correct way of putting
things.}
Moreover, by Lemma
\ref{toralmodification}, we also have that $\xi_{\varphi}$ and $\xi_{\varphi_{\hat{w}}}$ have
the same restriction to $\HT{-1}(\Gamma, T(L))$.  Therefore,
$\xi_{\varphi} \otimes \xi_{\varphi_{\hat{w}}}^{-1}$ is a character of $T(K)$.  Since
$\xi_{\varphi} \otimes \xi_{\varphi_{\hat{w}}}^{-1}$ is correct on $T(\OL)_{\Gamma}$, and
since $T(\OL)_{\Gamma} \xrightarrow{\Nm} T(K)$ is surjective,
we are done.\MAxxx{I think we need to make this proof more understandable.  It's not
very understandable right now....the last few sentences are not clear.}
\end{proof}
