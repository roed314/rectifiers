...
Since $\phi|_{W_L}$ has image in $\hat{T} = X^*(T) \otimes \mathbb{C}^{\times}$,
then by the local Langlands correspondence for tori one can canonically
associate to $\phi$ a character $\xi_{\varphi}$ of $T(L)_{\Gamma}$, 
...
reductive group $G$ (see \cite{reeder}).  Then $\phi$ has image in a group of
