...
an action of $\Gamma$.  %ADD REFERENCE TO THESIS
...
\begin{equation}
\label{eq:Fr_phii}
\phi_i(\gamma) = \Fr^i(\phi_0(\gamma))
\end{equation}
for all $\gamma \in \Gal(\bar{L}/L)$. The quotient $\xi' \xi^{-1}$ will then break up
analogously, allowing the computation of its value on $x \in T(L)$ given $\Nm(x)$.

We define $\phi_i$ separately on inertia and on $\Fr^n$.
Set $t = \phi'(\Fr)\phi(\Fr)^{-1} \in \hat{T}$ and set $\phi_i(\Fr^n) = \Fr^i(t)$.
The restriction of $\phi' \phi^{-1}$ to inertia is a map $\chi : \I \to \hat{T}$.
Let $\chi_0$ be an arbitrary $(1+q+\cdots+q^{n-1})$-th root of $\chi$, and define
$\phi_i$ on $\I$ to be $\Fr^i \circ \chi_0$.  The $\phi_i$ satisfy \eqref{eq:Fr_phii}
...
Now let $\xi_i$ be the character of $T(L)$ associated to $\phi_i$.
