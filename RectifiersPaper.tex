...
focusing on supercuspidal representations of $G(K)$.  Bushnell-Henniart \cite{bushnellhenniart},
DeBacker-Reeder \cite{debackerreeder}, Kaletha \cite{kaletha} and Reeder \cite{reeder}
...
is precisely what appears in Bushnell and Henniart \cite{bushnellhenniart2,bushnellhenniart}.
...
Adler \cite{adler}, Howe \cite{howe}, Bushnell-Henniart \cite{bushnellhenniart},
Bushnell-Kutzko \cite{bushnellkutzko}, and Yu \cite{yu}.
...
characteristic zero, $\OK$ its ring of integers,
$\PK$ the maximal ideal in $\OK$,
and $k$ will denote the residue field of $K$.  Fix
a uniformizer $\varpi$ of $K$.
...
We will write $\hat{T} \subset \hat{G}$ for the dual torus in the complex dual group of $G$ \cite[\S I.2]{borel}.
...
the case of unramified elliptic tori (see Definition \ref{admissiblepairhowe}).  Indeed,
...
Note that if $S_1$ is nonempty then we get $\alpha = 1$ immediately, so we may assume the contrary.  Then by the pigeonhole principle, there is a $w \in W(K)$ with $\# S_w > \# \Thinadm$.  Pick $\chi \in S_w$; since $\# S_w > \#\Thinadm$ there is some $\chi' \in S_w$ with $\frac{\chi}{\chi'}$ admissible.  We now have
...
$H \subset G$, it would be interesting to know whether the rectifiers for tori in $H$ and $G$ are related.  In particular, if $G = \GL_n$, this would allow us to apply the results of \cite{bushnellhenniart} in this setting.
\end{remark}
...
Corollary \ref{cor:cohom_tori}.  We need a basic result about powers of lifts of Coxeter elements in $\GL_{n}(\CC)$.
...
reciprocity map for $L$.  This, Proposition \ref{prop:powers_of_lifts}, and the local Langlands correspondence for tori imply that
...
\bibitem{borel}
   Armand Borel,
   \emph{Automorphic L-functions},
   Proc. Symp. Pure Math 33 vol. 2 (1979), AMS, pp. 27-62.

\bibitem{bushnellhenniart2}
  Colin Bushnell and Guy Henniart,
...
\bibitem{bushnellkutzko}
  Colin Bushnell and Phil Kutzko,
  \emph{The admissible dual of $GL(n)$ via compact open subgroups.} Ann. Math. Studies 129. Princeton Univ. Press 1993.
...
\bibitem{grossreeder}
  Benedict Gross and Mark Reeder,
...
\bibitem{moyprasad}
  ...
  \emph{Unrefined minimal $K$-types for $p$-adic groups},
