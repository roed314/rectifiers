...
$\GL_{n}(K)$.  Our generalization begins with is to unramified
connected reductive
...
from the Weil-Deligne group $\WD_K$, to the $L$-group ${}^L G$ of $G$.
...
central subject of their notion of a rectifier.
...
with the rectifier in the setting $\GL_{n}(K)$ as in \cite{bushnellhenniart}.
...
in $L$, containing $K$.  Then $L_0 = L_1 = \cdots L_{r-1} = L$ and $L_{j} = K$
for all $j \geq r$.  By Theorem ??????????? and Theorem ??????
...
Recall now the Tate cohomology sequence 
$$1 \rightarrow \HT{-1}(\Gamma,T(L)) \rightarrow T(L)_{\Gamma} \xrightarrow{\Nm} T(K)
= T(L)^{\Gamma} \rightarrow \HT{0}(\Gamma,T(L)) \rightarrow 1.$$
...
We have proven the existence of ${}_K  \mugen$ in Proposition 
\ref{existenceofrectifier}.  Namely, 
$G$ be an arbitrary unramified connected reductive group.  If
$T$ is an unramified torus in $G$, then $T$ can be defined
via Galois twisting by a Weyl group element $w$.  We defined
...
If $G$ is semisimple,
we proved that the function $$(T, \xi) \mapsto \xi_{\varphi_{w}}^{-1}$$
...
We will prove uniqueness in section ????
...
Moreover, in \S\ref{section:BH_compat}, we prove 
compatibility with Bushnell/Henniart:
...
$\xi_{\varphi_{w}} = \xi_{\varphi_{w}}^{-1} = {}_K \mu_{\xi}$.
\end{corollary}
...
\section{Uniqueness of weak rectifier} \label{section:DR_compat}

In this section, we prove the uniqueness part of Theorem \ref{thm:unique_semisimple}.
