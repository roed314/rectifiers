...
In fact, by Lemma \ref{lem:toral_modification}, $\xi_{\varphi_{\hat{w}}} = \xi_{\varphi'}$
...
consider groups that are not necessarily semisimple, such as $\GL_{n}(K)$ (see \S\ref{section:BH_compat}).

We also note that an unramified compact mod center torus might not have trivial $\HT{0}$, which is one reason why we assumed $G$ to be semisimple in Proposition \ref{prop:existenceofrectifier}.
...
\item The rectifier in our setting is a constant function.  We expect that for ramified tori, this will not be the case.
\item The behavior of rectifiers under change of group is not yet clear to us.  Perhaps two embeddings of a torus into
...
\begin{theorem} \label{thm:bh_agreement}
  If $G = \GL_{n}(K)$, the constant function $(T,\xi) \mapsto \mu$ agrees with
