...
$\Lpack(\chi)$ associated to characters $\chi$ of tori $T(K)$ in $G(K)$ (see
...
\item An admissible pair is in \emph{general position} if $\xi|_{T(1+\mathfrak{p}_L^{r})_{\Gamma}}$
...
\end{align*}
on $\Pgen(K)$, satisfying conditions (1)-(3) of Definition \ref{def:rectifier}.
\end{enumerate}
\end{definition}
...
unramified for all pairs $(T, \xi)$ in general position.
\end{theorem}
