...
Let $G$ be a connected reductive group, defined over a $p$-adic field $K$.  The local Langlands conjecture predicts a correspondence between a finite to one map, from the set of isomorphism classes of irreducible admissible representations of $G(K)$, to the set of equivalence classes of admissible homomorphisms from the Weil-Deligne group $W_K'$ of $K$, to the $L$-group ${}^L G$ of $G$.  

In the case of supercuspidal representations of $G(K)$, there has been a significant amount of progress in the local Langlands correspondence in recent years.  In the recent work of \cite{bushnellhenniart}, \cite{debackerreeder}, \cite{kaletha1}, and \cite{reeder}, the strategy of constructing a local Langlands correspondence is to first attach a character of an elliptic torus to a Langlands parameter $W_K \rightarrow {}^L G$, where $W_K$ is the Weil group of $K$, and then to construct a putative supercuspidal $L$-packet associated to this character.
...
with the rectifiers of $GL(n,K)$ as in \cite{bushnellhenniart}.  We recall the 
notion of a rectifier, as in \cite{bushnellhenniart}.
